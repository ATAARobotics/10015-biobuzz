...
public abstract class Auto extends RobotBaseOp {
    ...
    private final Pose blueFarShoot = new Pose(
            42.25 + xOffset,
            yOffset + 7.0, // inches in front of start position
            Math.toRadians(180)
    );
        ...
        auto.addCommands(
            ...
            new ParallelRaceGroup(
                ...
                new SequentialCommandGroup(
                        pathBetween(blueFarShoot, tiltedCorner, 0.70),
                        new Delay(0.5),
                        pathBetween(tiltedCorner, slightlyBack, 0.35),
                        new Delay(0.2)
                )
            ),
            new ParallelCommandGroup(
...
*/
        ...
        auto.addCommands(
            ...
            new ParallelRaceGroup(
                new AutoIntake(),
                new SequentialCommandGroup(
                    pathBetween(blueFarShoot, tiltedCorner, 0.85),
                    new Delay(2.0)
                )
            )
        );
    ...
    private Command nearPathing() {
        ...
        if (onlyPreloads){
            auto.addCommands(
                    new SequentialCommandGroup(
                            new Delay(15.0),
                            pathBetween(blueNearStart, finalShootAndPark, 1.0),
                            new AutoOuttake()
                    )
            );
            return auto;
